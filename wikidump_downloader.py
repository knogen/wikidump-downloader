...
parser.add_argument(
    "--data-path", type=str, default="./data/", help="the data directory"
)
parser.add_argument(
    "--proxies", type=str, default="", help="use the downloader proxies"
)
parser.add_argument(
    "--compress-type",
    type=str,
...
parser.add_argument(
    "--threads", type=int, default=3, help="number of threads [default: 3]"
)
parser.add_argument(
    "--start", type=int, default=1, help="the first file to download [default: 0]"
)
parser.add_argument(
    "--end", type=int, default=-1, help="the last file to download [default: -1]"
)
parser.add_argument(
    "--verify",
    action="store_true",
    default=False,
    help="verify the dump files in the specific path",
)
...
def download(dump_status_file, data_path, compress_type, start, end, thread_num):
    ...
    with open(dump_status_file) as json_data:
        ...
        history_dump = json.load(json_data)["jobs"][
            "metahistory" + compress_type + "dump"
        ]
...
def main():
    ...
    urllib.request.urlretrieve(
        f"https://dumps.wikimedia.org/enwiki/{version_flag}/dumpstatus.json",
        dump_status_file,
    )
